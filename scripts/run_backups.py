...
class BackupManager:
    ...
    def __init__(self, args):
        ...
        self.global_config = self.parse_config(
            self.args.config_filename, self.SYSTEM_CONFIG_FILENAME)
    ...
    @staticmethod
    def parse_config(config_filename, system_config_filename = ''):
        '''
        Parse config from a config file.
        '''
        parser = configparser.ConfigParser()
        parser.read_dict(BackupManager.get_default_config())
        parser.read([config_filename, system_config_filename])
        return parser
    ...
    def make_backups(self, section):
        ...
        shell_command = ['sudo', 'rsync', '-a', '--log-file',
                         section.get('logfile')]
        ...
        if len(section.get('exclude_file')) > 0:
...
def main():
    ...
    #  for section in config.sections():
        ...
        #  check_config(config_section)
        #  if run_backups(config_section, args):
            #  # If backups were successful, add an entry in the cache file
            #  history_writer.read_dict({section: {'last_backup':
                                                #  int(time.time())}})

    #  with open('/var/cache/ink/history', 'w') as history_file:
        #  history_writer.write(history_file)
def parse_args(argv):
    '''
    ...
    parser.add_argument('--system-config', dest='system_config_filename',
                        default='/etc/ink/inkrc', help='Path to system-wide '
                        'configuration file.')
